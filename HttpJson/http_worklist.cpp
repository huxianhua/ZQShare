...
void HTTP_WORKLIST::out_demo(QtHttpRequest * request, QtHttpReply * reply)
{
    ...
    reply->addHeader("Content-type","text/html;charset=utf-8");
    QString json =  SQL_RECORD::instance()->get_record_demo();
...
void HTTP_WORKLIST::out_default(QtHttpRequest * request, QtHttpReply * reply)
{
    ...
    reply->appendRawData (QByteArrayLiteral ("<h1>武汉中旗-iMac 系列心电图机 -- WorkList 服务程序</h1>\r\n"));
...
void HTTP_WORKLIST::s_result()
{
    if(SQL_RECORD::instance()->isValid())
    {
        m_resultVaild = true;
        m_json = SQL_RECORD::instance()->get_record_json();
    }else
    {
        m_resultVaild = false;
    }
